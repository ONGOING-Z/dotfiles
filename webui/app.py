...
def validate_install_conf():
    """对 install.conf.yaml 进行基本结构与路径校验，返回问题列表。"""
    issues: List[Dict[str, str]] = []
    try:
        if not INSTALL_CONF.exists():
            return {'valid': False, 'issues': [{'type': 'error', 'message': f'missing file: {INSTALL_CONF}'}]}

        with open(INSTALL_CONF) as f:
            conf = yaml.safe_load(f)

        if not isinstance(conf, list):
            issues.append({'type': 'error', 'message': 'root must be a list'})
        ...
        if link_map is None:
            issues.append({'type': 'warning', 'message': 'no link section found'})
            return {'valid': len(issues) == 0, 'issues': issues}

        if not isinstance(link_map, dict):
            issues.append({'type': 'error', 'message': 'link must be a mapping of target->source'})
            return {'valid': False, 'issues': issues}

        # 校验每一项
        for target, source in link_map.items():
            if not isinstance(target, str):
                issues.append({'type': 'error', 'message': f'invalid target key (not string): {repr(target)}'})
                continue
            # 展开 ~
            target_path = Path(os.path.expanduser(target))

            # 支持简单形式与包含 path 的 dict
            if isinstance(source, dict):
                src_path_str = source.get('path')
            else:
                src_path_str = str(source)

            if not src_path_str:
                issues.append({'type': 'error', 'message': f'source path missing for target {target}'})
                continue

            source_path = (BASE_DIR / src_path_str).resolve()

            # 基础建议
            if not target.startswith('/') and not target.startswith('~'):
                issues.append({'type': 'warning', 'message': f'target should be absolute or ~ prefixed: {target}'})

            if not source_path.exists():
                issues.append({'type': 'error', 'message': f'source not found: {source_path}'})
            elif source_path.is_dir():
                issues.append({'type': 'warning', 'message': f'source is a directory (ensure intended): {source_path}'})

            # 目标父目录存在性提示
            if not target_path.parent.exists():
                issues.append({'type': 'info', 'message': f'target parent will be created if missing: {target_path.parent}'})

        return {'valid': not any(i['type'] == 'error' for i in issues), 'issues': issues}
    except Exception as e:
        return {'valid': False, 'issues': [{'type': 'error', 'message': str(e)}]}


@app.route('/api/validate', methods=['GET'])
def api_validate():
    """校验 install.conf.yaml 基本结构与路径，返回问题列表。"""
    try:
        result = validate_install_conf()
        return jsonify(result)
    except Exception as e:
        return jsonify({'valid': False, 'issues': [{'type': 'error', 'message': str(e)}]}), 500
...
def git_commit():
    """提交变更。body: { message: 'commit message', files: ['file1', 'file2'] }"""
    try:
        body = request.json or {}
        message = body.get('message', 'Web UI commit')
        files = body.get('files', [])
        
        if not files:
            return jsonify({'error': 'no_files_specified'}), 400
            
        # 添加文件到暂存区
        add_result = subprocess.run(['git', 'add'] + files, 
                                  cwd=BASE_DIR, capture_output=True, text=True)
...
def git_reset():
    """回滚变更。body: { files: ['file1', 'file2'], hard: false }"""
    try:
        body = request.json or {}
        files = body.get('files', [])
        ...
        if hard:
            ...
            if not files:
                return jsonify({'error': 'no_files_specified'}), 400
            result = subprocess.run(['git', 'checkout', 'HEAD', '--'] + files, 
                                  cwd=BASE_DIR, capture_output=True, text=True)
